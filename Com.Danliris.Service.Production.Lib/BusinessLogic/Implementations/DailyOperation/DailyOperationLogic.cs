...
namespace Com.Danliris.Service.Finishing.Printing.Lib.BusinessLogic.Implementations.DailyOperation
{
    public class DailyOperationLogic : BaseLogic<DailyOperationModel>
    {
        ...
        public DailyOperationModel GetPreviousState(DailyOperationModel model)
        {
            if (model.Type.ToUpper() == "INPUT")
            {
                return DbSet.Where(dailyOperation => dailyOperation.KanbanId == model.KanbanId && dailyOperation.KanbanStepIndex == model.KanbanStepIndex - 1 && dailyOperation.Type.ToUpper() == "OUTPUT").SingleOrDefault();
            }
            else
            {
                return DbSet.Where(dailyOperation => dailyOperation.KanbanId == model.KanbanId && dailyOperation.KanbanStepIndex == model.KanbanStepIndex && dailyOperation.Type.ToUpper() == "INPUT").SingleOrDefault();
            }
        }
        ...
        public void CreateSnapshot(DailyOperationModel model)
        {
            ...
            var stepData = selectedKanban.Instruction.Steps
                .FirstOrDefault(s => s.Process == model.StepProcess && s.MachineId == model.MachineId && s.StepIndex == model.KanbanStepIndex);
        ...
        public void EditSnapshot(DailyOperationModel model)
        {
            var selectedKanban = DbSetKanban.Include(s => s.Instruction).ThenInclude(s => s.Steps)
               .Where(kanban => kanban.Id == model.KanbanId).SingleOrDefault();
            var stepData = selectedKanban.Instruction.Steps.FirstOrDefault(s => s.Process == model.StepProcess && s.MachineId == model.MachineId && s.StepIndex == model.KanbanStepIndex);
            int stepIndex = stepData.StepIndex;
